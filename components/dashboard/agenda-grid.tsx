    ...
    <div style={{ padding: isMobile ? "12px 14px 32px" : "24px 28px 40px", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "flex-end", justifyContent: "space-between", gap: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontFamily: "'Instrument Serif', Georgia, serif", fontSize: isMobile ? 20 : 25, lineHeight: 1.1, color: "#221F1B" }}>
              {formatDate(date)}
            </div>
            {!isMobile && <div style={{ fontSize: 13, color: "#928B7E" }}>{clubName} · {courts.length} canchas de pádel</div>}
          </div>
        </div>
      ...
      <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" as React.CSSProperties["WebkitOverflowScrolling"] }}>
        <div style={{ background: "#FCFBF8", border: "1px solid #E7E1D6", borderRadius: 16, padding: 14, minWidth: isMobile ? 520 : "auto" }}>
          ...
          <div style={{ display: "flex", gap: 8, padding: "2px 2px 12px", borderBottom: "1px solid #EFEAE0", marginBottom: 10 }}>
            ...
            {!isMobile && <div style={{ width: 132, flexShrink: 0, fontSize: 11, fontWeight: 700, letterSpacing: ".05em", textTransform: "uppercase", color: "#A39C8F", paddingTop: 2 }}>Disponibilidad</div>}
          ...
          {hasMorningClasses && (
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              ...
              <div style={{ flex: 1, display: "flex", alignItems: "center", padding: "12px 16px", borderRadius: 10, color: "#3D5C93", fontWeight: 600, fontSize: isMobile ? 12.5 : 13.5, background: "repeating-linear-gradient(45deg,#EAF0F8,#EAF0F8 11px,#E1E9F6 11px,#E1E9F6 22px)", border: "1px solid #D3DEF0" }}>
                Escuela de pádel · clases de 08 a 16 h
              </div>
            </div>
          )}
                    ...
                    return (
                      <div key={cell.courtId} onClick={() => setSelectedSlot({ start: row.start, end: row.end, cell, date })}
                        style={{
                          ...
                          border: `1px solid ${st.bd}`, borderRadius: 10, padding: "10px 10px",
                          minHeight: isMobile ? 60 : 74, display: "flex", flexDirection: "column",
                      ...
                      >
                        ...
                        {cell.who && (
                          <div style={{ fontSize: isMobile ? 12.5 : 14, fontWeight: 600, color: "#221F1B", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{cell.who}</div>
                        )}
                        <div style={{ fontSize: 11.5, color: "#8B857A", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{cell.status === "libre" ? "Disponible" : (cell.sub ?? "")}</div>
                      </div>
                    );
                  })}
                </div>
