...
6. **Endpoints legacy del boilerplate SaaS deshabilitados** (`jobs/create` → fal.ai, `jobs/result`,
   `jobs/status`, `stripe/checkout`, `stripe/portal`, `stripe/webhook`, `mercadopago/checkout` de
   créditos). Estaban deployados con `FAL_KEY`/`STRIPE_SECRET_KEY` configuradas y alcanzables por
   cualquier usuario autenticado → superficie de **gasto/abuso** (drenar fal.ai, exponer la cuenta
   central de pagos). Cancha no usa nada de esto (su cobro es marketplace por club vía OAuth). Ahora
   devuelven **404** salvo que se setee `ENABLE_LEGACY_SAAS="true"` (reversible). Helper en
   `lib/http/legacy.ts`.
